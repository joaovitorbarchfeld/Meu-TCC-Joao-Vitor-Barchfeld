...
import type {
  DispositivoCreateInput,
  DispositivoUpdateInput,
  DispositivoVincularInput,
  DispositivoValidarInput,
  DispositivoFilter,
} from '../validators/dispositivos.schemas';

export class DispositivosService {
  async list(filters: DispositivoFilter) {
    let query = db
    ...
    // Contar total
    let countQuery = db.selectFrom('dispositivos').select(db.fn.count('id').as('total'));
  ...
  async getById(id: string) {
    ...
    if (!dispositivo) {
      throw new AppError(404, 'Dispositivo não encontrado', 'DISPOSITIVO_NOT_FOUND');
    }

    return dispositivo;
  }

  async create(input: DispositivoCreateInput) {
    // Verificar se identificador já existe
    const identificadorExiste = await db
      .selectFrom('dispositivos')
      .select('id')
    ...
    if (identificadorExiste) {
      throw new AppError(409, 'Identificador já cadastrado', 'IDENTIFICADOR_DUPLICADO');
    }
    ...
    if (input.veiculo_id) {
      ...
      const veiculoTemDispositivo = await db
        .selectFrom('dispositivos')
        .select('id')
      ...
      if (veiculoTemDispositivo) {
        throw new AppError(409, 'Este veículo já possui um dispositivo vinculado', 'VEICULO_JA_TEM_DISPOSITIVO');
      }
    }
  ...
  async update(id: string, input: DispositivoUpdateInput) {
    // Verificar se existe
    ...
    if (input.identificador) {
      ...
      if (identificadorExiste) {
        throw new AppError(409, 'Identificador já cadastrado em outro dispositivo', 'IDENTIFICADOR_DUPLICADO');
      }
    }
  ...
  async delete(id: string) {
    ...
    await db.deleteFrom('dispositivos').where('id', '=', id).execute();

    return { message: 'Dispositivo excluído com sucesso' };
  }

  async vincular(id: string, input: DispositivoVincularInput) {
    const dispositivo = await db
      .selectFrom('dispositivos')
    ...
    if (!dispositivo.ativo) {
      throw new AppError(400, 'Dispositivo está inativo', 'DISPOSITIVO_INATIVO');
    }
    ...
    if (veiculoTemDispositivo) {
      throw new AppError(409, 'Este veículo já possui outro dispositivo vinculado', 'VEICULO_JA_TEM_DISPOSITIVO');
    }
  ...
  async desvincular(id: string) {
    ...
    if (!dispositivo.veiculo_id) {
      throw new AppError(400, 'Dispositivo não está vinculado a nenhum veículo', 'DISPOSITIVO_NAO_VINCULADO');
    }
  ...
  async validar(input: DispositivoValidarInput) {
    // Buscar dispositivo
    const dispositivo = await db
      .selectFrom('dispositivos')
