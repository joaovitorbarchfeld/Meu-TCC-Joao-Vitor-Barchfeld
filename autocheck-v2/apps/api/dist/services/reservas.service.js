...
exports.reservasService = exports.ReservasService = void 0;
const db_1 = require("../config/db");
const errorHandler_1 = require("../middlewares/errorHandler");
class ReservasService {
    async list(filters, usuarioLogadoId) {
        let query = db_1.db
            .selectFrom('reservas')
        ...
        // Contar total
        let countQuery = db_1.db.selectFrom('reservas').select(db_1.db.fn.count('id').as('total'));
        if (filters.start)
            countQuery = countQuery.where('end_at', '>=', new Date(filters.start));
        if (filters.end)
            countQuery = countQuery.where('start_at', '<=', new Date(filters.end));
        if (filters.veiculo_id)
            countQuery = countQuery.where('veiculo_id', '=', filters.veiculo_id);
        if (filters.usuario_id)
            countQuery = countQuery.where('usuario_id', '=', filters.usuario_id);
        if (filters.status === 'ativa') {
    ...
    async getById(id) {
        const reserva = await db_1.db
            ...
            .executeTakeFirst();
        if (!reserva) {
            throw new errorHandler_1.AppError(404, 'Reserva não encontrada', 'RESERVA_NOT_FOUND');
        }
        return reserva;
    }
    async create(input, usuarioLogadoId) {
        ...
        if (!veiculo.ativo) {
            throw new errorHandler_1.AppError(400, 'Veículo está inativo', 'VEICULO_INATIVO');
        }
        // Verificar overlap
        const hasOverlap = await this.checkOverlap(input.veiculo_id, new Date(input.start_at), new Date(input.end_at));
        if (hasOverlap) {
            throw new errorHandler_1.AppError(409, 'Já existe uma reserva para este veículo no período solicitado', 'RESERVA_OVERLAP');
        }
        // Criar reserva
        const reserva = await db_1.db
            .insertInto('reservas')
    ...
    async update(id, input, usuarioLogadoId, perfil) {
        // Buscar reserva
        const reservaExistente = await db_1.db
            .selectFrom('reservas')
            .selectAll()
            .where('id', '=', id)
            .executeTakeFirst();
        ...
        if (reservaExistente.usuario_id !== usuarioLogadoId &&
            perfil !== 'gestor' &&
            perfil !== 'admin') {
            throw new errorHandler_1.AppError(403, 'Sem permissão para editar esta reserva', 'SEM_PERMISSAO');
        }
        // Não permitir editar reservas passadas
        if (new Date(reservaExistente.end_at) < new Date()) {
            throw new errorHandler_1.AppError(400, 'Não é possível editar reservas passadas', 'RESERVA_PASSADA');
        }
        ...
        if (input.start_at || input.end_at) {
            ...
            const hasOverlap = await this.checkOverlap(reservaExistente.veiculo_id, novoStart, novoEnd, id);
            if (hasOverlap) {
                throw new errorHandler_1.AppError(409, 'Já existe outra reserva para este veículo no novo período', 'RESERVA_OVERLAP');
            }
        }
        // Atualizar
        const reserva = await db_1.db
    ...
    async delete(id, usuarioLogadoId, perfil) {
        const reserva = await db_1.db
            .selectFrom('reservas')
            .selectAll()
            .where('id', '=', id)
        ...
        // Verificar permissão
        if (reserva.usuario_id !== usuarioLogadoId &&
            perfil !== 'gestor' &&
            perfil !== 'admin') {
            throw new errorHandler_1.AppError(403, 'Sem permissão para cancelar esta reserva', 'SEM_PERMISSAO');
        }
        // Não permitir cancelar reservas em andamento ou passadas
        const agora = new Date();
        if (new Date(reserva.start_at) <= agora) {
            throw new errorHandler_1.AppError(400, 'Não é possível cancelar reservas em andamento ou passadas', 'RESERVA_INICIADA');
        }
        await db_1.db.deleteFrom('reservas').where('id', '=', id).execute();
        // Registrar auditoria
        await db_1.db
            .insertInto('auditoria_logs')
            .values({
    ...
    async getMinhasReservas(usuarioId, status = 'todas') {
        const filters = {
            usuario_id: usuarioId,
            status,
            page: 1,
    ...
    async checkOverlap(veiculoId, start, end, excludeReservaId) {
        let query = db_1.db
            .selectFrom('reservas')
            .select('id')
...
exports.reservasService = new ReservasService();
